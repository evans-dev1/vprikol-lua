...
function json()
    ...
    function list:save(array)
        ...
        if array and type(array) == 'table' and encodeJson(array) then
			...
			sms('Îøèáêà ïðè ñîõðàíåíèå ôàéëà!')
		end
    end
	...
	function(player)
        ...
        imgui.Begin('Frame', window, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoTitleBar)
            imgui.PushFont(fonts[30])
                if menu['page'] == 1 then -->> Îñíîâíàÿ ñòðàíèöà
                    if menu['information']['nick'] then
                        imgui.PushFont(fonts[20])
                                if method['page'] == 1 then
                                    imgui.SetCursorPosY(10)
                                    local arr = menu['information']['data']
                                    imgui.FTextCenter(u8( ('{Text}Èíôîðìàöèÿ î {ButtonActive}%s{Text} íà ñåðâåðå {ButtonActive}%s{Text} [{ButtonActive}%s{Text}]:'):format(menu['information']['nick'], server['list'][menu['information']['server']], menu['information']['server']) ))
                                    imgui.NewLine()
                                    imgui.BeginGroup()
                                        imgui.FTextCenter(u8('{Text}ID àêêàóíòà: {ButtonActive}' .. arr['accountId']), 300)
                                        imgui.FTextCenter(u8('{Text}Óðîâåíü: {ButtonActive}' .. arr['lvl']), 300)
                                        imgui.FTextCenter(u8('{Text}Óðîâåíü VIP: {ButtonActive}' .. arr['vipLabel']), 300)
                                        imgui.FTextCenter(u8('{Text}Íîìåð òåëåôîíà: {ButtonActive}' .. arr['phoneNumber']), 300)
                                        imgui.FTextCenter(u8('{Text}Ñîñòîÿíèå: {ButtonActive}' .. (arr['isOnline'] and 'Îíëàéí' or 'Îôôëàéí')), 300)
                                    imgui.EndGroup()
                                    imgui.SameLine()
                                    imgui.BeginGroup()
                                        imgui.FTextCenter(u8('{Text}Âñåãî äåíåã: {ButtonActive} $' .. money_separator(arr['totalMoney'])), 900)
                                        imgui.FTextCenter(u8('{Text}Íàëè÷íûå: {ButtonActive} $' .. money_separator(arr['cash'])), 900)
                                        imgui.FTextCenter(u8('{Text}Äåíüãè â áàíêå: {ButtonActive} $' .. money_separator(arr['bank'])), 900)
                                        imgui.FTextCenter(u8('{Text}Äåïîçèò: {ButtonActive} $' .. money_separator(arr['deposit'])), 900)
                                        imgui.FTextCenter(u8('{Text}Ëè÷íûé ñ÷åò: {ButtonActive}' .. (arr['individualAccount'] or 'Îòñóòñòâóåò')), 900)
                                    imgui.EndGroup()
                                    imgui.NewLine()
                                    imgui.FTextCenter(u8('{Text}Ðàáîòà: {ButtonActive}' .. arr['jobLabel']))
                                    imgui.FTextCenter(u8('{Text}Îðãàíèçàöèÿ: {ButtonActive}' .. arr['orgLabel']))
                                    imgui.FTextCenter(u8('{Text}Äîëæíîñòü: {ButtonActive}' .. (arr['rankLabel'] or 'Îòñóòñòâóåò')))
                                elseif method['page'] == 2 then
                                    imgui.PushFont(fonts[20])
                                        if menu['graph']['name'] then
                                            imgui.SetCursorPosY((imgui.GetWindowHeight() - 260) / 2)
                                            imgui.FTextCenter(u8(('{Text}Èìÿ {ButtonActive}%s{Text} %sÿâëÿåòñÿ ÐÏ.'):format(menu['information']['name']['value'], menu['information']['name']['rp'] and '' or 'íå ')), imgui.GetWindowWidth() * 0.5)
                                        ...
                                        if menu['graph']['surname'] then
                                            imgui.SetCursorPosY((imgui.GetWindowHeight() - 260) / 2)
                                            imgui.FTextCenter(u8(('{Text}Ôàìèëèÿ {ButtonActive}%s{Text} %sÿâëÿåòñÿ ÐÏ.'):format(menu['information']['surname']['value'], menu['information']['surname']['rp'] and '' or 'íå ')), imgui.GetWindowWidth() * 1.5)
                            ...
                            if imgui.CustomButton(u8('Âåðíóòüñÿ íàçàä'), imgui.ImVec2(300)) then menu['information'] = {} end
                        imgui.PopFont()
                    elseif menu['loading']['bool'] or #server['list'] == 0 then
                        if #server['list'] == 0 then
                            ...
                            imgui.FTextCenter(u8('Çàãðóçêà ñïèñêà ñåðâåðîâ...'))

                            imgui.SetCursorPosX((imgui.GetWindowWidth() - 350) / 2)
                            if imgui.CustomButton(u8('Ïîâòîðèòü ïîïûòêó'), imgui.ImVec2(350)) then getServerList() end
                        else
                    ...
                    elseif menu['captcha'] then
                        ...
                        imgui.BeginGroup()
                            ...
                            imgui.PushItemWidth(197.5); imgui.InputTextWithHint('##captcha', u8('Ââåäèòå êàï÷ó'), method['captchaInput'], ffi.sizeof(method['captchaInput'])); imgui.PopItemWidth()

                            imgui.SameLine()

                            if imgui.CustomButton(u8('Ïîäòâåðäèòü'), imgui.ImVec2(197.5)) then
                                getPlayerInformation(u8:decode(ffi.string(method['searchNick'])), server['selected'][0] + 1, u8:decode(ffi.string(method['captchaInput'])))
                                log('Êàï÷à ââåäåíà: ' .. u8:decode(ffi.string(method['captchaInput'])))
                                menu['loading']['bool'], menu['captcha'] = true, nil
                                imgui.StrCopy(method['captchaInput'], '')
                            end

                            if imgui.CustomButton(u8('Íàçàä'), imgui.ImVec2(400)) then
                                log('Ââîä êàï÷è îòìåí¸í!')
                                menu['captcha'] = nil
                            end
                        imgui.EndGroup()
                    else
                        if method['page'] == 1 then
                            ...
                            imgui.BeginGroup()
                                imgui.BetterInput('page1', u8('Ïîèñê èãðîêà'), 0, method['searchNick'], imgui.ImVec4(0.26, 0.59, 0.98, 1.00), imgui.ImVec4(0.00, 0.00, 0.00, 1.00), 300, imgui.ImVec4(0.5, 0.5, 0.5, 1.00))
                                ...
                                if imgui.CustomButton(u8('Íàéòè'), imgui.ImVec2(300, 40)) and #u8:decode(ffi.string(method['searchNick'])) >= 3 then
                        ...
                        elseif method['page'] == 2 then
                            ...
                            imgui.BeginGroup()
                                imgui.BetterInput('page2', u8('Ïîèñê èìåíè'), 0, method['searchNick'], imgui.ImVec4(0.26, 0.59, 0.98, 1.00), imgui.ImVec4(0.00, 0.00, 0.00, 1.00), 300, imgui.ImVec4(0.5, 0.5, 0.5, 1.00))
                                if imgui.CustomButton(u8('Íàéòè'), imgui.ImVec2(300, 40)) and #u8:decode(ffi.string(method['searchNick'])) >= 1 then
                                    menu['loading']['bool'] = true
                ...
                elseif menu['page'] == 2 then -->> Íàñòðîéêè
                    ...
                    imgui.BeginChild('hotkey', imgui.ImVec2(550, 140), false)
                        ...
                        imgui.SameLine((240 + 550 - imgui.CalcTextSize(u8('Èíôîðìàöèÿ î èãðîêå')).x) / 2)
                        imgui.Text(u8('Èíôîðìàöèÿ î èãðîêå'))

                        imgui.Hotkey('##checkrp', 'checkrp', imgui.ImVec2(235))
                        imgui.SameLine((240 + 550 - imgui.CalcTextSize(u8('Ïðîâåðêà ÐÏ íèêà')).x) / 2)
                        imgui.Text(u8('Ïðîâåðêà ÐÏ íèêà'))

                        if imgui.CustomButton(u8('Âåðíóòüñÿ íàçàä'), imgui.ImVec2(-1)) then menu['page'] = 1 end
                    imgui.EndChild()
                elseif menu['page'] == 3 then -->> Âûáîð ìåòîäîâ
                    imgui.SetCursorPos(imgui.ImVec2((imgui.GetWindowWidth() - 350) / 2, (imgui.GetWindowHeight() - 130) / 2))
                    imgui.BeginGroup()
                        if imgui.ButtonSelected(u8('Èíôîðìàöèÿ î èãðîêå'), imgui.ImVec2(350, 40), 1) then method['page'] = 1 end
                        if imgui.ButtonSelected(u8('Ïðîâåðêà ÐÏ íèêà'), imgui.ImVec2(350, 40), 2) then method['page'] = 2 end
                        if imgui.Button(u8('Âåðíóòüñÿ íàçàä'), imgui.ImVec2(350)) then menu['page'] = 1 end
                    imgui.EndGroup()
                end

                imgui.PushFont(fonts[20])
                    imgui.SetCursorPos(imgui.ImVec2((imgui.GetWindowWidth() - imgui.CalcTextSize(u8('âåñåëûé ïðèêîë | https://vk.com/vprikolbot')).x) / 2, imgui.GetWindowHeight() - 25))
                    imgui.TextColored(imgui.ImVec4(0, 0, 0, 0.7), u8('âåñåëûé ïðèêîë | https://vk.com/vprikolbot'))
                imgui.PopFont()
            imgui.PopFont()
        imgui.End()
    end
        ...
        imgui.Begin('Update', _, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoTitleBar)
            imgui.PushFont(fonts[30])
                if update['data']['version'] then
                    imgui.Text(u8('Äîñòóïíî îáíîâëåíèå!'))
                    imgui.PushFont(fonts[20])
                        imgui.FText(u8('{TextDisabled}Òåêóùàÿ âåðñèÿ: #' .. thisScript().version))
                        imgui.FText(u8( ('{TextDisabled}Àêòóàëüíàÿ âåðñèÿ: #%s %s'):format(update['data']['version']['v'], update['data']['update']['mandatory'] and '{DragDropTarget}[Îáÿçàòåëüíîå]' or '') ))
                    ...
                    if not update['data']['update']['mandatory'] then
                        if imgui.CustomButton(u8('Óñòàíîâèòü'), imgui.ImVec2(200)) then downloadUpdate() end
                        imgui.SameLine(495)
                        if imgui.CustomButton(u8('Îòìåíà'), imgui.ImVec2(200)) then update['check'] = true end
                    else
                    ...
                    imgui.FTextCenter(u8('Ïðîâåðêà íàëè÷èÿ îáíîâëåíèé...'))

                    imgui.SetCursorPosX((imgui.GetWindowWidth() - 300) / 2)
                    if imgui.CustomButton(u8('Ïîâòîðèòü ïîïûòêó'), imgui.ImVec2(300)) then getScriptUpdate() end
                end
            imgui.PopFont()
        imgui.End()
    end
...
function onScriptTerminate(scr, quitGame) 
    if scr == thisScript() then
        log('Ñêðèïò çàâåðøèë ðàáîòó. Ïðè÷èíà: ' .. (quitGame and 'Âûõîä èç èãðû' or 'Ñöåíàðèé çàâåðøåí'))
    end
end
...
function imgui.Hotkey(label, keys, size)
    ...
    local result = false
    local name = UI_HOTKEY[label] and (os.time() % 2 == 0 and u8('Íàæìèòå êëàâèøó') or '') or (settings['hotkey'][keys] ~= -1 and vkeys.id_to_name(settings['hotkey'][keys]) or u8('Ñâîáîäíî'))
...
function getPlayerInformation(nick, currentServer, captcha)
    log('getPlayerInformation: ' .. ('%s | Ñåðâåð: %s [%s] | Êàï÷à: %s'):format(nick, server['list'][currentServer], currentServer, tostring(captcha)))
    ...
    asyncHttpRequest('GET', 'https://backend.vprikol.ru/api/find', params,
        function(response)
            ...
            if response.status_code == 200 then
                ...
                if decodeJson(response.text)['status'] == 'captcha' then
                    log('getPlayerInformation: Ïðèøëà êàï÷à')
                    ...
                    downloadUrlToFile(url, path_download, function(id, status, p1, p2)
                        if status == dlstatus.STATUS_ENDDOWNLOADDATA then
                            log('getPlayerInformation: Ñêà÷àë êàï÷ó')
                ...
                elseif decodeJson(response.text)['status'] == 'success' then
                    log('getPlayerInformation: Ïðèøëà èíôîðìàöèÿ')
        ...
        function(err)
            ...
            sms('Ïðîèçîøëà îøèáêà: {mc}' .. tostring(err):match('requests%.lua:%d+: (.+)'))
            menu['loading']['bool'] = false
        end) 
end
    ...
    for k, v in ipairs(image) do
        ...
        if v[2] then
            downloadUrlToFile(v[2], path, function(id, status, p1, p2)
                if status == dlstatus.STATUS_ENDDOWNLOADDATA then
                    log('downloadImage: ' .. v[1] .. ' Ñêà÷àë êàðòèíêó')
            ...
            log('downloadImage: ' .. v[1] .. ' Êàðòèíêà íå íóæíà')
            menu['graph'][v[1]] = nil
            countDownload = countDownload + 1
            if countDownload == 2 then resolve() end
        end
    ...
    downloadUrlToFile('https://api-samp.arizona-five.com/launcher/servers-data', path_download, function(id, status, p1, p2)
        if status == dlstatus.STATUS_ENDDOWNLOADDATA then
            ...
            if file then
                ...
                log('Ñïèñîê ñåðâåðîâ çàãðóæåí. Êîëè÷åñòâî: ' .. #server['list'])
            end
        end
    end)
end
    ...
    downloadUrlToFile('https://raw.githubusercontent.com/evans-dev1/vprikol-lua/main/version.json', path_download, function(id, status, p1, p2)
        if status == dlstatus.STATUS_ENDDOWNLOADDATA then
            ...
            if file then
                ...
                if t then
                    update['data'] = t
                    log('Ñïèñîê îáíîâëåíèé çàãðóæåí!')
                    log('Âåðñèÿ ñêðèïòà: ' .. thisScript().version)
                    log('Ïîñëåäíÿÿ âåðñèÿ: ' .. update['data']['version']['v'])
